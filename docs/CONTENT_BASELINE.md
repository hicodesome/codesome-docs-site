...
当前基准以站点仓库工作树为真值：根目录中符合 `^\d{2}-.+\.md$` 的公开文章，以及这些文章实际引用的本地 `images/` 图片。标题来自每篇文章第一行的唯一 canonical H1，文章、图片、引用关系、大小、SHA-256 和稳定内容指纹由 `docs/article-backup/manifest.json` 统一记录。指纹只计算公开文章/引用图片的稳定路径、内容哈希和引用关系，不读取 Git HEAD、commitDate 或运行时间。
...
该命令自动发现文章和图片、生成标题映射与后台配置、更新并清理 `docs/article-backup/`，最后执行当前基准检查。重复运行不写 Git HEAD、commitDate 或运行时间等不稳定字段；即使产生无关 Git 提交，文章和图片内容未变化时也不会重写 manifest。

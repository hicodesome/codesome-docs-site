...
`npm run check` 会启动本地生产服务，逐个请求 `index.html` 引用的本地脚本，并确认 `server.mjs`、`scripts/` 和 `docs/` 仍保持私有；随后用实际的标题注入器遍历全部文章，验证渲染输入恰好包含登记的正式 H1。因此，浏览器脚本路径、私有路径规则、标题映射或注入器行为发生回归时，会在提交和发布前失败。

...
* 下单页面用于购买套餐、月卡或充值，入口是 `https://meta.codesome.cn`（原 `fk.codesome.cn` 已停止维护，需用邮箱和密码注册新站独立账号）。
...
![502 报错示例](<images/codesome｜使用问题速查-codesome-502.png?v=458c33de1a0cb267668d6e96fe3ce8497833900140e0070a0dd18ccef2a8fa7c>)
...
1. 稍等几分钟后重试；如果是 Claude Code 长对话，先 `/compact` 压缩上下文，或重启客户端，排除会话状态影响。

2. 如果急用，可以临时购买按量付费，并切换到可用的按量分组。

如果你是按量分组：

1. 回到 API Key 页面切换分组，或换一个配置到可用分组的 key。

2. 更换网络（例如切换到手机热点）后再试一次。

如果多次重试仍然报 503：

* 保留报错截图、当前分组、使用的工具和网络环境信息，联系客服处理。

重要提醒：使用 ccswitch 等 API 管理工具时，不要随意自定义模型名称；模型名称保持默认或按教程填写。

下图用于检查 ccswitch 里是否误填了自定义模型名称。

![ccswitch 模型名称示例](<images/codesome｜使用问题速查-codesome-503-model-name.jpeg?v=95eaedf69ef36b27da15ad22a12c228161d0fec995a7f31a84af8ea7b7db4665>)
...
![ERR\_BAD\_REQUEST 报错示例](<images/codesome｜使用问题速查-anthropic-err-bad-request.png?v=60e3c386ec035d10d0a279846d04a8de2f94503c29136e4961ccb4122778d931>)
...
![ECONNRESET 报错示例](<images/codesome｜使用问题速查-econnreset-example.png?v=0c8a1f844c3125ecb304430b8548cafc131ae814a468e1d3d338155775d7ac8b>)
...
![Claude Code 登录卡住示例](<images/codesome｜使用问题速查-skip-claude-login.jpg?v=d9a7363cf5ca35983e5cc6abaad18ecf848e999bc5b10f0c945c673a961d110c>)
...
## 11. 如何快速删除 PowerShell 和环境变量配置里所有和 Claude / Anthropic 有关的变量
...
## 12. WSL 下 Codex 反复要求登录 / 启动失败（`thread/start failed during TUI bootstrap`）
...
> 用 npm 全局安装 Claude Code、Codex、OpenClaw 等客户端时，最常见的两类问题是：安装时报权限错误（无法写入 `/usr/local`）、装完却提示找不到命令。下面按平台给出修复步骤，不需要用 `sudo`。

### 13.1 先确认 npm 全局目录

打开终端（macOS / Linux 用终端，Windows 用 PowerShell），执行：

```bash
npm config get prefix
```

- 返回 `/usr/local`（macOS / Linux 常见）：npm 想装到系统目录，普通用户没有写入权限，所以安装时报权限错误（`EACCES: permission denied` 之类）。
- 返回 `%APPDATA%\npm`（Windows 常见）：目录本身可写，但它可能不在 PATH 里，所以装完找不到命令。

### 13.2 macOS / Linux：把 npm 全局目录改到用户目录

不要在 npm 安装命令前加 `sudo`。正确做法是让 npm 装到自己的用户目录：
...
- 终端用 zsh 时写 `~/.zshrc`；用 bash 时把 `~/.zshrc` 换成 `~/.bashrc`。
- **重新打开终端**，再执行安装命令并验证（以 Claude Code 为例，Codex / OpenClaw 同理）：
...
### 13.3 Windows：把 npm 全局路径加进 PATH

1. 打开 PowerShell，执行 `npm config get prefix`，记下路径（通常是 `%APPDATA%\npm`）。
2. 右键「此电脑」→「属性」→「高级系统设置」→「环境变量」。
3. 在「用户变量」里找到 `Path`，双击编辑，新增上面记下的 npm 全局路径。
4. 确定保存后**重新打开 PowerShell**，再执行安装与验证。

Windows 上如果只是安装 Claude Code 且没有特殊原因，优先用官方安装器（PowerShell 执行 `irm https://claude.ai/install.ps1 | iex`）；需要走 npm 的场景（如 Codex、OpenClaw）按上面步骤修复。

### 13.4 还是不行时

按上面步骤操作后仍然失败，可能是以下原因：

- 系统架构与安装包不匹配：执行 `node -p "process.platform + ' ' + process.arch"` 确认。
- Node.js 版本过旧或安装异常：重新安装 Node.js LTS 后再试。
- 公司网络策略或安全软件拦截了 npm 下载。
- Windows 内置 Administrator 账户（SID 末尾 -500）安装 Microsoft Store / MSIX 应用时报 `0x80070005` 权限拒绝：属于桌面版安装问题，优先改用 CLI 安装。

持续失败时，保留 `npm config get prefix` 输出和安装命令的完整报错截图联系客服。截图请隐藏 API Key、邮箱、订单号等身份信息。

## 14. 切换到 Max 分组后，为什么旧窗口里还是不生效？
...
## 15. Retrying in 5s · attempt 4/10（重连循环）
...
5. 如果以上都不行，稍等几分钟后重试，或按上文切换到可用的分组；持续报错时保留截图和网络环境信息联系客服。

示例截图：

![](<images/codesome｜使用问题速查-重连-大部分情况是您开启了vpn.png?v=e3b29b8badb2ce76abae56c4b8495339977e67beb9f8ccf2f9a6848eb6896354>)

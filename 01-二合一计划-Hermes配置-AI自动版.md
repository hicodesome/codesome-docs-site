# Hermes 自动配置（V3/V5 按产品选择）

> 本页的自动配置示例默认写的是 V3。二合一 V5 不能混用 V3 的域名或 Key：Codex 使用 `https://v5.codesome.cn/openai`，Claude 使用 `https://v5.codesome.cn/api`，二合一 Key 通常以 `cr-` 开头。需要手动配置 V5 时，请继续阅读本页的 Mac 小白版或二合一 Hermes 手动版。

》》[ codesome｜Claude code 小白无门槛宝典](https://zvgmnl1sw58.feishu.cn/wiki/Vaifwy0aAisdP8kDLPoc0jV5nCb?from=from_copylink) （点击扫码进群一起畅玩 cc）

# 有手就行，如何使用skill全自动配置hermes

首先安装 Trae 等 AI agent 软件。

本页前半部分给 AI 的 Hermes skill 使用 V3 配置；若你购买的是二合一，请把 skill 生成的 `base_url`、`api_key` 和环境变量按上面的 V5 地址替换，并在实际运行前执行 `hermes chat -q "Reply with exactly: HERMES_OK"` 验证模型路径。
...
- Claude 路径：Claude、`claude-sonnet-5`、`claude-opus-4-8`、`CODESOME_CLAUDE_API_KEY`、CodeSome Claude

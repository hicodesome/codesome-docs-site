...
export function parseArgs(argv = process.argv.slice(2)) {
  ...
  if (!config.article || !config.title || !config.expect) failUsage('--article, --title and --expect are required');
...
export function evaluateBrowserAssertions(probe, config) {
  ...
  return [
    ...
    {
      id: 'article-body',
      pass: normalizeText(article.bodyText).includes(normalizeText(config.expect)),
      detail: `正文包含关键文字：${config.expect}`
    },
    {
      id: 'article-images',
      pass: images.length > 0 && images.every(image => image.complete === true && Number(image.naturalWidth) > 0),
      detail: images.length ? `正文图片 ${images.length} 张，naturalWidth 均大于 0` : '正文没有可验收图片'
    },
    {
      id: 'article-console',
